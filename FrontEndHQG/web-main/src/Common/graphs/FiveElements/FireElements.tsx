import { select, svg } from 'd3'
import React, { useEffect } from 'react'

type FireElementsType = {
  appendToSelector: string
  svgRef: SVGSVGElement | null,
  dimensions: any
}

export const FireElements = ({ svgRef, dimensions, appendToSelector }: FireElementsType) => {

  useEffect(() => {
    if (!svgRef) {
      return
    }
    const svg = select(svgRef)

    const elg = svg.select(appendToSelector)

    const g = elg.append("g")
      .attr("id", "fire-g")
    // .attr("clip-path", "url(#allow-inside)")
    // .attr("mask", "url(#allow-outside)")

    g.append("circle")
      .attr("id", "fire")
      .attr("cx", 0)
      .attr("cy", 0)
      .attr("r", dimensions.width / 4 / 3)
      .attr("fill-opacity", 0)
      .attr("stroke", "green")
      .attr("stroke-width", 4)
      .on("mouseover", function (_, d) {
        console.log(appendToSelector);
        select(this).style('stroke-width', 10)
      })
      .on("mouseout", function () {
        select(this).style('stroke-width', 2)
      })

    // g.append("text")
    //   .attr("id", "fire-text")
    //   .attr("x", 0)
    //   .attr("y", 20)
    //   .attr("text-anchor", "middle")
    //   .text("hello")

    return () => {
      svg.selectAll("#fire-g").remove()
      svg.selectAll("#fire").remove()
      svg.selectAll("#fire-text").remove()
    }
  }, [svgRef, dimensions])


  return (
    <>
    </>
  )
}
