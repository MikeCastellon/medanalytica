...
const SignupSchema = Yup.object().shape({
  ...
  mobilePhone: Yup.string()
    .nullable() // Allows null or empty strings
    ...
    .test(
      ...
      (value) =>
        !value ||
        Yup.string().phone("Your country code", false).isValidSync(value)
    ),
...
type UserCreateUpdateFormProps = {
  onFormSubmit: (values: any, cb: () => void) => void;
  allowPassword?: boolean;
  initialValues?: InitialValues;
  additionalActions?: ReactNode | null;
  formSize?: "large" | "small";
};
        ...
        {({
          ...
          <Form className="flex flex-col mt-8 w-full px-2">
            <Field
              ...
              placeholder="Email"
              className={`flex ${
                formSize === "large" ? "text-sm" : "text-xs"
              } border-[1px] border-charcoal border-opacity-25 rounded-md p-2`}
            />
            {errors.email && touched.email ? (
              <div className="text-red-600">{errors.email}</div>
            ) : null}

            <div className={`${formSize === "large" ? "mt-6" : "mt-2"}`}></div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex flex-col flex-1 w-full">
                <Field
                  ...
                  className={`flex ${
                    formSize === "large" ? "text-sm" : "text-xs"
                  } flex-1 border-[1px] border-charcoal border-opacity-25 rounded-md p-2`}
                />
            ...
            <Field
              name="mobilePhone"
              type="string"
              placeholder="Mobile Phone eg. +1234567890"
              className={`flex ${
                formSize === "large" ? "text-sm" : "text-xs"
              } border-[1px] border-charcoal border-opacity-25 rounded-md p-2`}
            />
            {errors.mobilePhone && touched.mobilePhone ? (
              <div className="text-red-600">{errors.mobilePhone}</div>
            ) : null}
            <div className={`${formSize === "large" ? "mt-6" : "mt-2"}`}></div>
            ...
            <div className="flex sm:flex-row flex-wrap gap-4">
              <div className="">
                <Field
                  ...
                  className={`${
                    formSize === "large" ? "text-sm" : "text-xs"
                  } w-24 border-[1px] border-charcoal border-opacity-25 rounded-md p-2`}
                />
              </div>
              <div className="">
                <Field
            ...
            {allowPassword && usePassword && (
              <div className="flex flex-row">
                <Field
                  ...
                  className={`${
                    formSize === "large" ? "text-sm" : "text-xs"
                  } flex flex-1 border-[1px] border-charcoal border-opacity-25 rounded-md p-2`}
                />
                <button
                  type="button"
                  className="pl-2"
              ...
              <>
                ...
                <Field
                  ...
                  className={`${
                    ...
                    errors.confirmPassword ? "border-primary-red" : null
                  } ${
                    values.confirmPassword === "" ? "border-charcoal" : null
                  } ${
                    ...
                    values.confirmPassword !== ""
                      ...
                      : null
                  }  rounded-md p-2 focus:border-2 active:border-0 focus:outline-none  `}
                />
              </>
            )}
            {errors.confirmPassword ? (
            ...
            <div className="flex flex-row justify-end gap-2">
              ...
              <button
                ...
                className={`flex flex-row items-center px-2 ${
                  isValid ? "bg-primary-green" : "bg-primary-grey"
                } rounded-md ${
                  formSize === "large" ? "text-sm" : "text-xs"
                } text-white py-2  mt-2 mb-2`}
              >
                {isSubmitting ? (
                  <span>
                    <svg
