...
import { useGetDescriptionByKeyQuery } from '../../generated/graphql';
import { useAuth } from '../../Hooks/AuthContext';

type BasicModalElementProps = {
  title: string
  descriptionKey: string
  width?: "WIDE" | "NORMAL"
}

export const BasicModalElement = ({ title, descriptionKey, width = "NORMAL" }: BasicModalElementProps) => {
  const auth = useAuth()
  // const { isOpen, setClosed, descriptionElement } = useDescriptionModal()

  const { data, isLoading } = useGetDescriptionByKeyQuery({
  ...
  return (
    <div className={` flex flex-col w-full md:w-1/2   min-h-40 bg-white shadow-md rounded-lg  overflow-y-auto p-3 z-50`}>
      ...
      {isLoading ?
        ...
        <div  >
          <div className='text-sm text-primary-grey pb-4' dangerouslySetInnerHTML={{ __html: data?.descriptions[0]?.hq_professional ?? "Sorry could not get te decription" }}></div>
        </div>
      }
    </div>
