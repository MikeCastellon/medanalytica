...
export const UserRecordingMenuItem = ({ recording: { id, title, created_on, data, ecg_file, rr_file, trend_recording } }: { recording: TRecording }) => {
  const auth = useAuth()
  const router = useRouter<LocationGenerics>()
  const routeMatch = useMatch<LocationGenerics>()
  const location = useLocation()
  const currentPage = location.current.pathname.split("/")[6]
  // console.log("ROOTMATCH: ", currentPage);
  ...
  return (
    <Link
      ...
      {({ isActive }) => (
        <div className={`${isActive ? "bg-white  border-l-4  border-black" : ""} group flex flex-row h-14 px-2 pt-1 border-charcoal border-b-[1px]  hover:bg-desaturated-grey hover:bg-opacity-0 text-charcoal hover:text-black cursor-pointer hover:border-l-4  hover:border-black`}>
