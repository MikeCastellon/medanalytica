...
import { answer } from '../../Questionnaires/QuestionSetLayout'
import { useCreateAnswerGroupMutation, useGetQuestionnaireByIdQuery } from '../../generated/graphql'
...
export const FillQuestionnaireById = () => {
  ...
  const { mutate, isLoading: answerGroupLoading } = useCreateAnswerGroupMutation(dataSource, {
    ...
    onSuccess: () => {
      location.history.push(`/${userId}/questionnaires/questionnaires`)
    }
  })
    ...
    <div className=' p-8'>
      ...
      {/* {
        questionSet?.question_set_id?.id ?
          <TrendAnswerSets owner_id={userId} question_set_id={questionSet?.question_set_id?.id} />
          : null
      } */}
